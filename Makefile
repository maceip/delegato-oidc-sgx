...
SELF_EXE = target/release/delgato-oidc-sgx

.PHONY: all
all: $(SELF_EXE) delgato-oidc-sgx.manifest
ifeq ($(SGX),1)
all: delgato-oidc-sgx.manifest.sgx delgato-oidc-sgx.sig
endif
...
delgato-oidc-sgx.manifest: delgato-oidc-sgx.manifest.template
	gramine-manifest \
...
delgato-oidc-sgx.manifest.sgx delgato-oidc-sgx.sig: sgx_sign
	@:

.INTERMEDIATE: sgx_sign
sgx_sign: delgato-oidc-sgx.manifest $(SELF_EXE)
	gramine-sgx-sign \
		--manifest $< \
		--output $<.sgx
